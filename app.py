...
# Updated Filtering method to ensure exact condition matching
def generate_pandas_filter_code(model, user_prompt, df_columns):
    ai_prompt = f"""
    You are an expert in pandas, and your job is to generate filtering code for a DataFrame in pandas.

    The user wants to filter the data according to this prompt: '{user_prompt}'.
    The DataFrame has the following columns: {df_columns}.

    Generate the pandas code needed to filter this DataFrame exactly as per the user's request.
    Only output the code and ensure no explanation is included.
    """
...
def filter_data_with_pandas_code(df, pandas_code):
    try:
        # Execute the generated Pandas code to filter the DataFrame exactly as specified
...
# Existing chart and visualization functions remain the same.
...
def create_chart(chart_type, df, x_col, y_col, additional_params=None):
    ...
    if isinstance(color_col, list): color_col = color_col[0]
    if isinstance(size_col, list): size_col = size_col[0]
    try:
        if chart_type == "line":
...
def main():
    ...
    if 'chart_suggestions' not in st.session_state: st.session_state.chart_suggestions = None
    if 'selected_chart' not in st.session_state: st.session_state.selected_chart = None
